...
def convert_dataset(
    ...
    for idx, image in tqdm(enumerate(input_iter), total=num_files):
        ...
        elif dataset_attrs['width'] != cur_image_attrs['width']:
            err = [f'  dataset {k}/cur image {k}: {dataset_attrs[k]}/{cur_image_attrs[k]}' for k in dataset_attrs.keys()] # pylint: disable=unsubscriptable-object
            error(f'Image {archive_fname} attributes must be equal across all images of the dataset.  Got:\n' + '\n'.join(err))
        elif dataset_attrs['height'] != cur_image_attrs['height']:
            err = [f'  dataset {k}/cur image {k}: {dataset_attrs[k]}/{cur_image_attrs[k]}' for k in dataset_attrs.keys()] # pylint: disable=unsubscriptable-object
            error(f'Image {archive_fname} attributes must be equal across all images of the dataset.  Got:\n' + '\n'.join(err))
        elif dataset_attrs['channels'] != cur_image_attrs['channels']:
            # convert img with 3 channels to image with 1 channel
            if cur_image_attrs['channels'] == 1:
